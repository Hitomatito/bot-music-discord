...
def _candidate_search_queries(query: str) -> list[str]:
    ...
    if not artist_query:
        ...
        candidate_queries.append(f"{cleaned_query} topic")
...
def _candidate_playlist_search_queries(query: str) -> list[str]:
    ...
    if len(tokens) >= 3:
        ...
        if first_two and after_first_two:
            candidate_queries.append(f'{first_two} {after_first_two} {first_two} playlist')
        if last_two and before_last_two:
            candidate_queries.append(f'{last_two} {before_last_two} {last_two} playlist')
...
async def search_public_youtube_playlist(query: str, limit: int = 5):
    ...
    for candidate_query in candidate_queries:
        try:
            result_set = await asyncio.to_thread(_search_duckduckgo_sync, candidate_query, limit)
        ...
        if candidate_playlists:
            return max(candidate_playlists, key=lambda entry: _score_playlist_result(cleaned_query, entry))

    return None


def _score_youtube_result_for_pair(title_query: str, artist_query: str, entry: dict) -> float:
    ...
    title_text = _normalize_search_text(entry.get("title", ""))
    uploader_text = _normalize_search_text(entry.get("uploader") or entry.get("channel") or "")
    ...
    artist_query_tokens = [token for token in artist_query_text.split() if len(token) > 1]
    ...
    artist_title_overlap = sum(1 for token in artist_query_tokens if token in title_tokens)
    artist_uploader_overlap = sum(1 for token in artist_query_tokens if token in uploader_tokens)
    title_uploader_overlap = sum(1 for token in title_query_tokens if token in uploader_tokens)
    ...
    if artist_query_text and (artist_query_text in uploader_text or artist_query_text in title_text):
        score += 8.0
    ...
    if artist_query_text and query_text and query_text in title_text and artist_uploader_overlap == 0:
        score -= 10.0
    ...
    official_markers = ("official", "video oficial", "topic")
    cover_markers = ("cover", "letra", "lyrics", "lyric", "karaoke", "instrumental", "speed", "nightcore", "tutorial")
    live_markers = ("en vivo", "live")
    ...
    if artist_query_text and any(marker in title_text for marker in live_markers):
        score -= 6.0
    ...
    if not artist_query_text:
        ...
        if len(query_tokens) == 1 and title_words and title_words[0] == query_tokens[0]:
            if len(title_words) > 1:
                ...
                if second_word in {"audio", "oficial", "official", "video", "topic"}:
                    score += 10.0
                else:
                    score -= 8.0

        if len(query_tokens) == 1:
            song_segment = raw_title.split(" - ", 1)[1] if " - " in raw_title else raw_title
...
def _score_youtube_result(query: str, entry: dict) -> float:
    return max(_score_youtube_result_for_pair(title_query, artist_query, entry) for title_query, artist_query in _candidate_music_query_pairs(query))
...
async def search_youtube_best_match(query: str, limit: int = 8):
    ...
    candidate_queries = _candidate_search_queries(query)
    search_results = await asyncio.gather(
        *(search_youtube(candidate_query, limit=limit) for candidate_query in candidate_queries)
    )
    ...
    return max(candidate_videos, key=lambda entry: _score_youtube_result_for_best_match(query, entry))
...
def is_youtube_url(text: str) -> bool:
    ...
    return host in {'youtube.com', 'youtu.be', 'youtube-nocookie.com', 'music.youtube.com'}
